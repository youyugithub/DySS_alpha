...
List joint_local_linear(arma::cube yyijk,
                        arma::icube ttijk,
                        arma::imat nobs,
                        arma::ivec st,
                        arma::ivec ot,
  ...
  const arma::uword I0=0,I1=ndim-1,II0=ndim,II1=2*ndim-1;
  ...
  arma::vec mub(ndim2,arma::fill::zeros),mub_old(ndim2,arma::fill::zeros);
  arma::mat Sigmab(ndim2,ndim2,arma::fill::eye);
  arma::mat Sigmab_inv(ndim2,ndim2,arma::fill::eye);
  ...
  arma::vec Ebbi(ndim2,arma::fill::zeros);
  arma::mat Ebbibbit(ndim2,ndim2,arma::fill::eye);
  arma::mat Eeeeeeet(ndim,ndim,arma::fill::eye);
  
  arma::vec Eyexp(ndim,arma::fill::zeros);
  arma::mat Eyyexp(ndim,ndim,arma::fill::zeros);
  
  arma::vec Ebi(ndim2,arma::fill::zeros);
  arma::mat Vbi(ndim2,ndim2,arma::fill::eye);
  arma::mat Vbi_half(ndim2,ndim2,arma::fill::eye);
  ...
  arma::vec avec(ndim2);
  arma::mat amat(ndim2,ndim2);
  
  double X0,X1,X2;//,X3,X4
  double XY0,XY1;//,XY2
  double SUMX0,SUMX1,SUMX2;//,SUMX3,SUMX4
  arma::vec XKSKY(ndim2);
  arma::mat XKSKX(ndim2,ndim2);
  double temp_double;
  int temp_int;
  arma::vec temp_ndim(ndim);
  arma::vec temp_ndim2(ndim2);
  arma::mat temp_ndim_ndim(ndim,ndim);
  arma::mat temp_ndim2_ndim2(ndim2,ndim2);
  ...
  double sum_for_nind;
  double ee;
  arma::vec sum_for_Ebi(ndim2);
  arma::mat sum_for_Sigmab(ndim2,ndim2);
      ...
      for(ii=0;ii<nind;ii++){
        ...
        for(kk=0;kk<ndim;kk++){
          for(jj=0;jj<nobs(ii,kk);jj++){
            ...
            // -ttdiff_double*ttdiff_double*Ebi(III0+kk);

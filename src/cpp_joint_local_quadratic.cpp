...
List joint_local_quadratic(arma::cube yyijk,
                           arma::icube ttijk,
                           arma::imat nobs,
                           arma::ivec st,
                           arma::ivec ot,
  ...
  arma::vec mub(ndim3,arma::fill::zeros),mub_old(ndim3,arma::fill::zeros);
  arma::mat Sigmab(ndim3,ndim3,arma::fill::eye);
  arma::mat Sigmab_inv(ndim3,ndim3,arma::fill::eye);
  ...
  arma::vec Ebbi(ndim3,arma::fill::zeros);
  arma::mat Ebbibbit(ndim3,ndim3,arma::fill::eye);
  arma::mat Eeeeeeet(ndim,ndim,arma::fill::eye);
  
  arma::vec Eyexp(ndim,arma::fill::zeros);
  arma::mat Eyyexp(ndim,ndim,arma::fill::zeros);
  
  arma::vec Ebi(ndim3,arma::fill::zeros);
  arma::mat Vbi(ndim3,ndim3,arma::fill::eye);
  arma::mat Vbi_half(ndim3,ndim3,arma::fill::eye);
  ...
  arma::vec XKSKY(ndim3);
  arma::mat XKSKX(ndim3,ndim3);
  double temp_double;
  int temp_int;
    ...
    for(ii=0;ii<nind;ii++){
      ...
      for(kk=0;kk<ndim;kk++){
        ...
        for(jj=0;jj<nobs(ii,kk);jj++){
          ...
          XKX(III0+kk,I0+kk)=XKX(III0+kk,I0+kk)+ttker(ttdiff+hh);
          XKX(I0+kk,II0+kk)=XKX(I0+kk,II0+kk)+tker(ttdiff+hh);
          XKX(II0+kk,II0+kk)=XKX(II0+kk,II0+kk)+ttker(ttdiff+hh);
          XKX(III0+kk,II0+kk)=XKX(III0+kk,II0+kk)+tttker(ttdiff+hh);
          XKX(I0+kk,III0+kk)=XKX(I0+kk,III0+kk)+ttker(ttdiff+hh);
          XKX(II0+kk,III0+kk)=XKX(II0+kk,III0+kk)+tttker(ttdiff+hh);
          XKX(III0+kk,III0+kk)=XKX(III0+kk,III0+kk)+ttttker(ttdiff+hh);
          
          XKY(I0+kk)=XKY(I0+kk)+ker(ttdiff+hh)*yyijk(ii,jj,kk);
          XKY(II0+kk)=XKY(II0+kk)+tker(ttdiff+hh)*yyijk(ii,jj,kk);
          XKY(III0+kk)=XKY(III0+kk)+ttker(ttdiff+hh)*yyijk(ii,jj,kk);
  ...
  double sum_for_nind;
  double ee;
  arma::vec sum_for_Ebi(ndim3);
  arma::mat sum_for_Sigmab(ndim3,ndim3);
      ...
      for(ii=0;ii<nind;ii++){
        ...
        for(kk=0;kk<ndim;kk++){
          for(jj=0;jj<nobs(ii,kk);jj++){
            ...
            temp_double=temp_double+Vbi(III0+kk,I0+kk)*ttker(hh+ttdiff);
            temp_double=temp_double+Vbi(I0+kk,II0+kk)*tker(hh+ttdiff);
            temp_double=temp_double+Vbi(II0+kk,II0+kk)*ttker(hh+ttdiff);
            temp_double=temp_double+Vbi(III0+kk,II0+kk)*tttker(hh+ttdiff);
            temp_double=temp_double+Vbi(I0+kk,III0+kk)*ttker(hh+ttdiff);
            temp_double=temp_double+Vbi(II0+kk,III0+kk)*tttker(hh+ttdiff);
            temp_double=temp_double+Vbi(III0+kk,III0+kk)*ttttker(hh+ttdiff);
